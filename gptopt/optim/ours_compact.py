...
@torch.compile
def PolarExpress(G: torch.Tensor, steps, frob_eps=1e-2, deflation_eps=1e-2, centered=True):
    ...
    hs = (our_coeffs_list if centered else our_coeffs_list_uncentered)[:steps] + list(repeat(our_coeffs_list[-1], steps - len(our_coeffs_list)))
    for a, b, c in hs:
        a = a / (1 + deflation_eps)
        b = b / (1 + deflation_eps)
        c = c / (1 + deflation_eps)
