...
def optimizer_config(full_config):
    c = dict(name=full_config.get('optimizer_params', {}).get('name'), **full_config.get('optimizer_params', {}).get('args', {}))
...
def justnormalstuff(selected_run_data, outfolder):
    metric = "val_losses"
...
def namemeplease(run_data, outfolder):
    outfolder.mkdir(parents=True, exist_ok=True)
...
def main(experiment_name, figures_dir):
    ...
    for group_ix, (key, run_data) in enumerate(runs_by_group.items()):
        ...
        print(make_summary_df(run_data))
        namemeplease(run_data, group_out_path)
        justnormalstuff(run_data, group_out_path)
