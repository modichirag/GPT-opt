...
## Running experiments
- **Before submitting any sweep or batch job**, read `docs/guides/disbatch_pipeline.md` to understand the submission pipeline, task file generation, and logging structure.
...
- `run_hydra.py` enforces a clean git state before experiment runs (rejects dirty working tree).
- Before submitting experiments, check if there are uncommitted changes. If the changes affect optimizer code, training logic, or model code (i.e., anything that could affect reproducibility), **commit first** — flag the user and propose a commit message.
- If the only changes are non-consequential (param_configs, docs, .gitignore, notebooks, train scripts with only param defaults changed), use `ALLOW_DIRTY=1` to bypass the check.
- Never silently submit jobs that will fail due to dirty git — always check and resolve first.
