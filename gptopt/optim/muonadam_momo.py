...
class MuonAdamMomo(torch.optim.Optimizer):
    """
    Arguments:
    ...
    def step(self, closure=None, loss=None):
        ...
        for p in group["params"]:
            ...
            if self.use_truncation:
                ...
                if state["muon"]:
                    ...
                    global_dual_norm += torch.linalg.vector_norm(
                        m * torch.abs(m) / (eps + v.sqrt()), ord=1
                    )
        ...
        if self.use_truncation:
            ...
            truncated_lr = (self.loss_model - self.truncate_loss + new_loss_model.item()) / global_dual_norm
